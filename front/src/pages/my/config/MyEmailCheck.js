...
function MyEmailCheck({errors, watch, register, member}) {
    ...
    let [lastEmail, setLastEmail] = useState(member.email);
    return <tr>
        <th><span className="essential"></span>EMAIL</th>
        <td><input type="email" name="km_email" placeholder="이메일 입력" defaultValue={member.email}
                   {...register("email", {
            ...
            {emailCheck && lastEmail!==member.email&& !errors.email && <div style={{color: 'blue'}}>이메일 인증이 완료되었습니다.</div>}
        </td>
    </tr>
}
